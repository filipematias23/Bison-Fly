...
2. Download orthomosaics and unzip them into your current working directory (save all files in the same directory)
3. Draw the grid of plot polygons and generate a Shapefile or geopackage
4. Create a soil mask to evaluate canopy cover
5. Run a loop to extracting data for 14 flights in (UAV traits: calculating vegetation indices and plant height)
...
The first step will be to make sure all of the required libraries for the tutorial are installed and then load them.
...
Next, we can download, unzip, and look at the data.
...
#####################################
#download.file(path/to/NDSU_WheatData_UAV_2021.zip)
#unzip("NDSU_WheatData_UAV_2021.zip")

### List of orthomosaics ###
Field <- list.files("NDSU_WheatData_UAV_2021/5band/", full.names = TRUE) # 15 5band orthomosaics
Field_RGB <- list.files("NDSU_WheatData_UAV_2021/RGB/", full.names = TRUE) # 15 RGB orthomosaics
Field_DSM <- list.files("NDSU_WheatData_UAV_2021/DSM/", full.names = TRUE) # 15 DSM orthomosaics

stopifnot(length(Field_RGB) == 15 &&
            length(Field_DSM) == 15 && 
            length(Field) == 15)
######################################################
...
### 3. Generate a map of plot polygons

The plot polygons can be drawn by selecting at least four points at the corners of the experiment. The number of columns and rows must be specified as arguments to the `fieldShape` function. At this point, the experimental borders can be eliminated as showing in the example bellow. 

The plot polygons are provided in the data repository as both a "shapefile" (Grid-Polygons-Plot.zip) and a "geopackage" (Grid-Polygons-Plot.geopackage). These were created using the steps below. These steps are interactive, and require you to locate the corners of the fields.

A geopackage is an Open Geospatial Consortium standards-compliant file type; you can learn more at the website [geopackage.org](https://www.geopackage.org/). A geopackage is a SQLite database with a specific schema that contains the same information as a shapefile, and is OGC-Compliant. A shapefile is actually a collection of files, and is a widely used but proprietary format developed by ESRI, the company that makes ArcGIS.
...
Data <- read.csv("NDSU_WheatData_UAV_2021/EX_DATA.csv",
                 header = TRUE,
                 fileEncoding = "UTF-8-BOM")
Map <- read.csv("NDSU_WheatData_UAV_2021/EX_MAP.csv",
                header = FALSE,
                fileEncoding = "UTF-8-BOM")
```

Next (optional), generate and save the polygons using an interactive plot. This is optional because the plot polygon file is provided in the data archive, but the code to demonstrate how to do it is shown below: 

```{r eval=FALSE}
## Display the field in a viewer. NB: this does not run in Rmarkdown, it must be run on the console to allow user input 
Shapefile <- fieldShape(mosaic = Test,
                        ...
                        ID = "PLOT",
                        fieldMap = Map)
rgdal::writeOGR(obj = Shapefile$fieldShape,
                dsn = 'NDSU_WheatData_UAV_2021/shapefile',
                layer = 'Grid-Polygons-Plot',
                driver = "ESRI Shapefile")

rgdal::writeOGR(obj = Shapefile$fieldShape,
                dsn = 'NDSU_WheatData_UAV_2021/Grid-Polygons-Plot.gpkg',
                layer = 'Grid-Polygons-Plot',
                driver = "GPKG")
```
...
Shapefile <- rgdal::readOGR('NDSU_WheatData_UAV_2021/Grid-Polygons-Plot.gpkg')
{ # wrapping in curly braces to avoid error 
  # https://stackoverflow.com/a/41947860/199217
  plotRGB(FIELDimageR:::RGB.rescale(Test, 3))
...
for(i in 2:length(Field)){
  print(paste("===",Field[i],"==="))
  EX <- stack(paste("./5band/", 
                    Field[i], 
                    sep = ""))
  EX.1 <- fieldMask(EX, plot = FALSE)
  EX.I <- fieldIndex(mosaic = EX, 
                     Red = 3,
                     Green = 2,
                     Blue = 1,
                     RedEdge = 4,
                     NIR = 5,
                     index = c("NGRDI", "BGI", "GLI", "NDVI", "NDRE", "CIG"," CIRE"), 
                     plot = FALSE)
  ...
  EX.I <- fieldArea(mosaic = EX.1$mask, 
                  fieldShape = EX.I$fieldShape, plot = FALSE)
  # EPH
